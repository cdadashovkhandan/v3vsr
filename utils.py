...
def add_batch_dims(fun, times=1, in_axes=0, out_axes=0):
...
def interpolate_grid(coords, grid, order=0):
    """
    args:
        coords: Tensor of shape (B, T, H, W, 2) with coordinates in [-0.5, 0.5]
        grid: Tensor of shape (B, T, H', W', C)
    returns:
        Tensor of shape (B, T, H, W, C) with interpolated values
    """
    size = grid.shape[-2]
    # convert [-0.5, 0.5] -> [0, size], where pixel centers are expected at
    # [-0.5 + 1 / (2*size), ..., 0.5 - 1 / (2*size)]
    coords = coords.transpose((0, 1, 4, 2, 3)) * size + (size - 1) / 2
    map_coordinates = partial(jax.scipy.ndimage.map_coordinates, order=order, mode='nearest')
    return jax.vmap(jax.vmap(jax.vmap(map_coordinates, in_axes=(2, None), out_axes=2)))\
            (grid, coords)
...
def j2t(a):
    return torch.from_numpy(np.asarray(a).copy()).permute(0, 3, 1, 2)
